...
class WindowsDevelopmentExecutor:
    ...
    def __init__(self, codex, project, output, *, codex_home, environment, baseline):
        ...
        if not all(p.is_absolute() and p.exists() for p in (self.codex, self.project, self.output, self.home)):
    ...
    def run(self, argv, *, mode="workspace-write", timeout=90, cancel=None, stdin=b""):
        ...
        if (not isinstance(argv, tuple) or not 1 <= len(argv) <= 64
                ...
                or not Path(argv[0]).is_absolute() or not 0 < timeout <= 300):
            raise ValueError("bounded literal argv and timeout required")
        packet = self.output / ("command-" + uuid.uuid4().hex)
        packet.mkdir()
        temporary = packet / "temp"
        temporary.mkdir()
        filesystem = {str(self.project): "write" if mode == "workspace-write" else "read",
                      str(temporary): "write"}
        profile = "excubitor_development"
        parent = ":workspace" if mode == "workspace-write" else ":read-only"
        policy = '{ extends = ' + json.dumps(parent) + ', filesystem = { ' + ", ".join(
            json.dumps(k) + "=" + json.dumps(v) for k, v in filesystem.items()
        ) + ' }, network = { enabled = false } }'
        server = [str(self.codex), "app-server", "--stdio", "-c", 'windows.sandbox="elevated"',
                  "-c", 'approval_policy="never"', "-c", 'mcp_servers={}',
                  "-c", 'web_search="disabled"', "-c", "default_permissions=" + json.dumps(profile),
                  "-c", f"permissions.{profile}=" + policy,
                  "-c", "log_dir=" + json.dumps(str(packet / "logs"))]
        for feature in ("plugins", "apps", "memories", "browser_use", "in_app_browser", "computer_use", "multi_agent"):
            server += ["--disable", feature]
        command = {"command": list(argv), "cwd": str(self.project), "permissionProfile": profile,
                   "timeoutMs": int(timeout * 1000)}
        request = packet / "request.json"
        request.write_text(json.dumps({"baseline": BASELINE, "server": server, "command": command,
                                      "events": str(packet / "events.jsonl")}), encoding="utf-8")
        # The authenticated coordinator environment is never passed through wholesale.
        allowed = {"SYSTEMROOT", "WINDIR", "COMSPEC", "PATHEXT", "SYSTEMDRIVE", "PROGRAMFILES",
                   "PROGRAMFILES(X86)", "PROGRAMDATA", "ALLUSERSPROFILE", "USERPROFILE", "LOCALAPPDATA",
                   "APPDATA", "USERNAME", "USERDOMAIN", "PATH"}
        env = {k: v for k, v in self.environment.items() if k.upper() in allowed}
        env.update(CODEX_HOME=str(self.home), CODEX_SQLITE_HOME=str(self.home),
                   TEMP=str(temporary), TMP=str(temporary), TMPDIR=str(temporary), PYTHONDONTWRITEBYTECODE="1")
        started = time.monotonic()
        outer = WindowsProcessTree().run((sys.executable, "-I", "-B", str(Path(__file__).resolve()), str(request)),
                                        self.project, env=env, timeout=timeout, cancelled=cancel,
                                        output_limit=2 * 1024 * 1024, terminate_on_root_exit=True)
        ...
        if (outer.execution.exit_code != 0 or outer.cancelled or outer.execution.timed_out
                or outer.execution.output_limited or not outer.drained):
            return outer
        try:
            native = json.loads(outer.execution.stdout)
            if type(native["exitCode"]) is not int or not all(type(native[k]) is str for k in ("stdout", "stderr")):
        ...
        execution = Execution(native["exitCode"], *captured, time.monotonic() - started,
