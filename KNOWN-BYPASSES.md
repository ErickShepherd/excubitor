...
and the `eval` builtin (`eval git push`, `eval "git push"`), which are stepped over / re-scanned the
same way (CLOSED table). The set is a curated list of launchers
...
| A **shell reserved word / grouping token** or the **`eval` builtin** hid the fenced verb one token deeper in the SAME segment — `eval git push`, `eval "git push"`, `{ git push; }`, `! git push`, `if git push; then :; fi`, `while git push; do …` — so the guard anchored `exe` on `!`/`{`/`if`/`eval` (which are keywords, not external launcher binaries, so the launcher step-over never fired), matched nothing, and deferred. Same no-privilege disarm class as the launchers above; worked in both `=1` and `=yolo`. Found by a fresh post-merge security audit, 2026-07-16 | Denied; `_classify` skips a leading run of `_SHELL_KEYWORD_PREFIXES` (`!`,`{`,`}`,`if`,`then`,`elif`,`else`,`fi`,`while`,`until`,`do`,`done`,`for`,`case`,`esac`,`select`,`in`) and re-scans `eval`'s JOINED args before anchoring `exe` (`time` stays a `_LAUNCHERS` binary — it takes options). Pinned in `TestShellKeywordAndEvalPrefix` | `guard-loop-vc.py::_classify` (post-merge audit) |
