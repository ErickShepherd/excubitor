...
committed its one unit. Subject-and-scope-matched (not a raw HEAD count) so the stage-a‖stage-b parallel
...
def _scoped_commit_count(repo_dir: str | None, scope: str) -> int | None:
    ...
    scope would inflate that stage's count and could cross-trip the stage-a‖stage-b parallel guarantee.
