...
def main():
    ...
    origin, candidate, metadata, evidence, temporary = (
        root / n for n in ("origin", "candidate", "git", "evidence", "temp")
    )
    ...
    for field in ("name", "email"):
        result = WindowsProcessTree().run(
            (str(git), "config", "user." + field), Path(__file__).resolve().parents[2], env=env, timeout=10
        )
    ...
    def retain(run, paths):
        ...
        git_command(
            ...
            "commit",
            "-qm",
            "Retain native development candidate",
        )
...
""",
    ...
    executor = WindowsDevelopmentExecutor(
        args.codex, candidate, evidence, codex_home=args.codex_home, environment=env, baseline=args.baseline
    )
    failed = executor.run(oracle.argv, mode="read-only", timeout=90)
    if (
        failed.execution.exit_code != 1
        or b"AssertionError" not in failed.execution.stderr
        or not failed.drained
    ):
    ...
    contract = Contract(
        ...
        "do not mutate the input. Preserve all other files. "
        "Run the supplied acceptance command using Python -B.",
        ("Fix median and execute its native tests.",),
        (oracle.check,),
        3,
        int(time.time()) + 720,
    )
    run = store.start(
        contract, "Owner said Do it for the explicit native-development-v1 slice; retain local branch only."
    )
    ...
    report = {
        ...
        "owner_starts": 1,
        "intermediate_owner_interventions": 0,
    }
    try:
        # An actual boundary rejection, not a prompt telling the worker to behave.
        try:
            runtime.apply(
                {"files": [{"path": "../authority/contract.json", "content": "forged"}], "command": []}
            )
        except RunError:
        ...
        report.update(
            ...
            frozen_check_unchanged=True,
            ordinary_task_unarmed=True,
            ordinary_command_passed=True,
            completed_run_does_not_restart=True,
        )
