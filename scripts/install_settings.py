...
- **Ownership is a parsed command target, not a substring.** An entry/handler is Excubitor-owned iff
  a shlex token of its command has an Excubitor guard script as its exact basename. A user's
  `xguard-loop-vc.py.bak` is not ours; our script at a stale path is.
...
def _command_target(command: str) -> str | None:
    """The Excubitor guard script a command targets: the first shlex token whose exact BASENAME is
    one of our scripts. None for every non-Excubitor command — substring hits are not ownership."""
    try:
